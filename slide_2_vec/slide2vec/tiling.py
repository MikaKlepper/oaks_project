...
def get_args_parser(add_help: bool = True):
    ...
    parser.add_argument("--run-id", type=str, default="", help="Name of output subdirectory")
    return parser
...
# def process_slide(*, wsi_path: Path, cfg, mask_visualize_dir, tile_visualize_dir, num_workers: int = 4):
#     """Process a single slide: extract tile coordinates and visualize if needed."""
#     wsi_name = wsi_path.stem.replace(" ", "_")
#     try:
#         tissue_mask_visu_path = None
...
#     except Exception as e:
#         return str(wsi_path), {"status": "failed", "error": str(e), "traceback": str(traceback.format_exc())}
def process_slide(
    *,
    wsi_path: Path,
    mask_path: Path,
...
def main(args):
    ...
    wsi_paths, _ = load_csv(cfg)

    parallel_workers = min(mp.cpu_count(), cfg.speed.num_workers_tiling)
    if "SLURM_JOB_CPUS_PER_NODE" in os.environ:
        parallel_workers = min(parallel_workers, int(os.environ["SLURM_JOB_CPUS_PER_NODE"]))
    ...
    if not skip_tiling:
        ...
        for wsi_path, status_info in results:
            process_df.loc[process_df["wsi_path"] == wsi_path, "tiling_status"] = status_info["status"]
            if "error" in status_info:
                process_df.loc[process_df["wsi_path"] == wsi_path, "error"] = status_info["error"]
                process_df.loc[process_df["wsi_path"] == wsi_path, "traceback"] = status_info["traceback"]

        process_df.to_csv(process_list, index=False)

        total_slides = len(process_df)
        slides_with_tiles = [str(p) for p in wsi_paths if Path(coordinates_dir, f"{p.stem}.npy").is_file()]
        failed_tiling = process_df[process_df["tiling_status"] == "failed"]
        no_tiles = process_df[~process_df["wsi_path"].isin(slides_with_tiles)]
        print("=+=" * 10)
        print(f"Total number of slides: {total_slides}")
