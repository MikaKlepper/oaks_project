...
def initialize_wandb(cfg: DictConfig, key: Optional[str] = ""):
    command = f"wandb login {key}"
    subprocess.call(command, shell=True)
    tags = [] if cfg.wandb.tags is None else [str(t) for t in cfg.wandb.tags]
    config = OmegaConf.to_container(cfg, resolve=True, throw_on_missing=True)
    if cfg.wandb.resume_id:
        run = wandb.init(
            project=cfg.wandb.project,
...
def load_csv(cfg):
    """Load CSV with only slide paths, no mask support."""
    df = pd.read_csv(cfg.csv)
    if "wsi_path" in df.columns:
        wsi_paths = [Path(x) for x in df.wsi_path.values.tolist()]
    elif "slide_path" in df.columns:
        ...
        raise ValueError("CSV must contain either 'wsi_path' or 'slide_path' column")
    # Always return None for mask paths
    mask_paths = [None for _ in wsi_paths]
    return wsi_paths, mask_paths
