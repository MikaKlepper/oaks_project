...
class TileDataset(torch.utils.data.Dataset):
    ...
    def load_coordinates(self, tile_dir):
        ...
        self.x = coordinates["x"][:407]
        self.y = coordinates["y"][:407]
        self.coordinates = (np.array([self.x, self.y]).T).astype(int)
        self.scaled_coordinates = self.scale_coordinates()
        self.tile_level = coordinates["tile_level"][:407]
        self.tile_size_resized = coordinates["tile_size_resized"][:407]
        resize_factor = coordinates["resize_factor"][:407]
