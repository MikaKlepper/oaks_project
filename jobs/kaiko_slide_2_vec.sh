...
#SBATCH --job-name="l-kaiko"
#SBATCH --output=/data/temporary/mika/repos/oaks_project/logs/slurm-%j-l-kaiko.out
...
echo "RUNNING SLIDE2VEC WITH KAIKO ON VALIDATION SET"

# paths
REPO_DIR="/data/temporary/mika/repos/oaks_project/slide_2_vec"
CONFIG_PATH="$REPO_DIR/yaml_configs/liver_kaiko.yaml"
...
SCRATCH_OUTPUT_DIR="$SCRATCH_BASE/outputs/KAIKO"
FINAL_OUTPUT_DIR="/data/temporary/toxicology/TG-GATES/liver/Validations_FM/KAIKO"
...
# setup environment
export HF_TOKEN="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"
...
echo "Pre-downloading Kaiko model to shared cache ..."
python3 - <<'PY'
import torch
torch.hub.load(
    "kaiko-ai/towards_large_pathology_fms",
    "vitl14",   # or "vitb16", "vits16" etc. — whichever you plan to use
    trust_repo=True
)
PY
echo "Kaiko model preloaded successfully!"
...
echo " Slide2Vec KAIKO job completed successfully."
